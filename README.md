...
iOS / iPadOS の **Dynamic Type** に対応しています。「設定 → 画面表示と明るさ → テキストサイズ」または「設定 → アクセシビリティ → 表示とテキストサイズ → さらに大きな文字」で文字サイズを変えると、サイト全体の文字とそれに付随する要素（章番号のバッジ、アイコン、カードの折り返し幅など）が連動して拡大・縮小します。

実装は CSS の `-apple-system-body` を `:root` の文字サイズに適用し、スタイルシート側の寸法をすべて `rem`（1rem = 17px = Dynamic Type の既定サイズ）で組む方式です。既定設定では従来とまったく同じ表示になり、ユーザーが設定を変えたときだけ追従します。

デスクトップのブラウザでは、ブラウザ自身の「フォントサイズ」設定に追従します（ルートを `106.25%` と相対指定しているため）。
